...
public class BanHistoryCommand extends Command {
    ...
    public void execute(CommandSender commandSender, String[] args) {
        ...
        if (!player.hasPermission(BanSystem.getInstance().getConfig().getString("permissions.banHistoryCommand"))) {
        ...
        if (args.length == 1) {
            ...
            if (documents.isEmpty()) {
                player.sendMessage(BanSystem.getInstance().getMessages().getString("messages.neverBanned"));
                return;
            }
            player.sendMessage(BanSystem.getInstance().getMessages().getListAsString("messages.banHistory")
                    .replace("%name%", name)
                    ...
                    .replace("%banned%", (dataBase.isBanned(uuid) ? "§a✔" : "§c✖"))
            ...
            for (Document document : documents) {
                ...
                if (!document.getString("unban").equalsIgnoreCase(""))
                    player.sendMessage(BanSystem.getInstance().getMessages().getString("messages.historyUnban")
                            .replace("%unban%", document.getString("unban").replace("-", " ")));

                player.sendMessage("");
            }
        } else {
            player.sendMessage(BanSystem.getInstance().getMessages().getString("messages.banHistorySyntax"));
        }
    }
}
